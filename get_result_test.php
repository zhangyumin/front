    ...
    <body>
            ...
            <?php
                ...
                echo"step3: 序列比对";
                    ...
                    {
                        $cmd3="./src/bowtie2-2.2.4/bowtie2 -L 25 -N 0 -i S,1,1.15 --no-unal -x ./src/bowtie2-2.2.4/indexes/bwt2_TAIR10 -q -U ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa -S ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam 2>&1";
                        #echo $cmd3;
                    }
                    else{
                        $cmd3="./src/bowtie2-2.2.4/bowtie2 -L 25 -N 0 -i S,1,1.15 --no-unal -x ./src/bowtie2-2.2.4/indexes/bwt2_TAIR10 -q -U ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa -S ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam 2>&1";
                ...
                echo "step4:获取polyA位点";
                 ...
                 if($_SESSION['rip']=='yes')
                 {
                    ...
                    $cmd5="./src/perl/PAT_setIP.pl -itbl /var/www/html/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA -otbl /var/www/html/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA -flds 0:1:2 -format file -conf ./db.xml 2>&1";
                    #echo $cmd5;
                 ...
                 echo"step6:导入PA表到数据库";
                     $cmd6="./src/perl/PAT_alterPA.pl -master db_user.PA_".$_SESSION['file']." -aptbl '/var/www/html/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA' -apsmp  $value -format file -conf ./db.xml 2>&1";
                    #echo $cmd6;
               ...
               $cmd7=$cmd7origin.$cmd7plus." -conf ./db.xml  2>&1";
                echo $cmd7;
                $out7=  shell_exec($cmd7);
                echo"<pre>$out7</pre>";

                echo"step8:提取序列并计算单核苷分布 ";
                $cmd8="./src/perl/PAT_trimSeq.pl -tbl pac_arab10 -cond  \"tot_tagnum>=2\" -suf $file_real[0].PAT2 -conf ./db.xml -opath './result/".$_SESSION['file']."/'  2>&1";
                #echo $cmd8;
                $out8=  shell_exec($cmd8);
                echo"<pre>$out8</pre>";
                $cmd9="./src/perl/PAS_kpssm.pl -seqdir \"./result/".$_SESSION['file']."/\" -pat \"$file_real[0].PAT2.*s$\" -from 1 -to 400 -k 1 -sort F -cnt T -freq T -tran T -suffix _atcg  2>&1";
                #echo $cmd9;
                $out9=  shell_exec($cmd9);
                echo"<pre>$out9</pre>";

             echo"step9:计算polyA信号";
                $cmd10="./src/perl/PAS_kcount.pl -seqdir \"./result/".$_SESSION['file']."/\" -pat \"$file_real[0].PAT2.*s$\" -k 6 -from 265 -to 290 -sort T -topn 50 -gap_once \"-1\" 2>&1" ;
                $out10=  shell_exec($cmd10);
                echo"<pre>$out10</pre>";
                
                 
                 #PAT导入jbrowse显示
//                 shell_exec("cp ./data/".$_SESSION['file']."/".$_SESSION['file'].".qc.fa.noT.fa.sam.M30S10.PA ./tojbrowse/pat.txt");//移动文件
//                 ///shell_exec("chown 777 ./tojbrowse/pat.txt");
