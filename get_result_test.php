...
//                $file_num = sizeof($file_name);
                ...
                foreach($upload_name as $key => $value)
                {
                    ...
                    $cmd1="fastq_quality_filter -q ".$_SESSION['qct']." -p ".$_SESSION['mp']." -v -Q 33 -i ./data/".$_SESSION['file']."/$value.fastq -o ./data/".$_SESSION['file']."/$value.qc.fa  2>&1";
                    #echo $cmd1;
                ...
                //echo "step2:去除polyT/polyA tail";与物种无关
                    ...
                    {
                        $cmd2="./src/perl/MAP_filterPolySeq.pl -s \"./data/".$_SESSION['file']."/$value.qc.fa\" -if fq -tl 8 -tr 20 -poly T -ml ".$_SESSION['minlength']." -qc T -of fq -st .noT.fa 2>&1";
                        #echo $cmd2;
                    }
                    else if($_SESSION['tailremove']=='A')
                    {
                        $cmd2="./src/perl/MAP_filterPolySeq.pl -s \"./data/".$_SESSION['file']."/$value.qc.fa\" -if fq -tl 8 -tr 20 -poly A -ml ".$_SESSION['minlength']." -qc T -of fq -st .noT.fa 2>&1";
                    }
                ...
                //echo"step3: 序列比对";
                    ...
                    {
                        $cmd3="./src/bowtie2-2.2.4/bowtie2 -L 25 -N 0 -i S,1,1.15 --no-unal -x ./src/bowtie2-2.2.4/indexes/bwt2_".$_SESSION['species']." -q -U ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa -S ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam 2>&1";
                        #echo $cmd3;
                    }
                    else{
                        $cmd3="./src/bowtie2-2.2.4/bowtie2 -L 25 -N 0 -i S,1,1.15 --no-unal -x ./src/bowtie2-2.2.4/indexes/bwt2_".$_SESSION['species']." -q -U ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa -S ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam 2>&1";
                ...
                //echo "step4:获取polyA位点";与物种无关
                    $cmd4="./src/perl/PAT_parseSAM2PA_II.pl -sam ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam -m 30 -s 10 -ofile ./data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA 2>&1 ";
                    #echo $cmd4;
                 ...
                 if($_SESSION['rip']=='yes')
                 {
                    ...
                    $cmd5="./src/perl/PAT_setIP.pl -itbl /var/www/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA -otbl /var/www/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA -flds 0:1:2 -format file -conf ./config/db_".$_SESSION['species'].".xml 2>&1";
                    #echo $cmd5;
                 ...
                 //echo"step6:导入PA表到数据库";
                    $cmd6="./src/perl/PAT_alterPA.pl -master db_user.PA_".$_SESSION['file']." -aptbl '/var/www/front/data/".$_SESSION['file']."/$value.qc.fa.noT.fa.sam.M30S10.PA' -apsmp  $value -format file -conf ./config/db_".$_SESSION['species'].".xml 2>&1";
                    #echo $cmd6;
...
//                $cmd7origin="./src/perl/PAT_PA2PAC.pl -d ".$_SESSION['distance']." -remap T -mtbl db_user.PA_".$_SESSION['file']." -gfftbl db_server.t_".$_SESSION['species']."_gff -anti T -otbl db_user.PAC_".$_SESSION['file']." -smps ";
               ...
               $cmd7=$cmd7origin.$cmd7plus." conf=\"/var/www/front/config/db_".$_SESSION['species'].".xml\"  2>&1";
//               echo $cmd7; 
               $out7=  shell_exec($cmd7);
                echo"<pre>$out7</pre>";

                //echo"step8:提取序列并计算单核苷分布 ";
                $cmd8="./src/perl/PAT_trimSeq.pl -tbl db_user.PAC_".$_SESSION['file']." -cond  \"tot_tagnum>=2\" -suf ".$_SESSION['file'].".PAT2 -conf ./config/db_".$_SESSION['species'].".xml -opath './result/".$_SESSION['file']."/'  2>&1";
                #echo $cmd8;
                $out8=  shell_exec($cmd8);
                echo"<pre>$out8</pre>";
                $cmd9="./src/perl/PAS_kpssm.pl -seqdir \"./result/".$_SESSION['file']."/\" -pat \"".$_SESSION['file'].".PAT2.*s$\" -from 1 -to 400 -k 1 -sort F -cnt T -freq T -tran T -suffix _atcg  2>&1";
                #echo $cmd9;
...
//                 $cmd11="select chr,strand,coord,$tag_num from pac_arab10 into outfile '/var/www/front/tojbrowse/pac.txt' ";
//                 //echo $cmd11;
//                 mysql_query($cmd11);
//                 shell_exec("./tojbrowse/txt2bed");
//                 $cmd12="/var/www/jbrowse/bin/flatfile-to-json.pl --bed /var/www/front/tojbrowse/pac.bed --trackLabel usrPac --out /var/www/html/jbrowse/data/";
//                 //echo $cmd12;
