...
# ogene=T/F(default); if T, 输出每种样本下的.long/short的基因列表
...
#<intbl>.<suffix>.avgPAT5.root_5daysXroot_60days.FU.long.gene -- 基因列表
...
for (i in 1:(ng-1)) {
  for (j in (i+1):ng) { 
	...
	#对所有.fu文件，作一个统计： （used gene#,switch gene#,switch gene%,A-Long,B-Long）
	  ngene=nrow(fu)
