...
!function($){
  ...
  var defaults = {
    ...
    this.css("top", buttonY  - 130  + "px");
    //this.css("left", buttonX - (this.outerWidth() / 2)   + "px");
    this.css("left", buttonX - 130  + "px");
    return this;
  }
  ...
  $.fn.flyIn = function (el, button, width, height, angle, step, radius, settings) {
	...
	var x = Math.round(width/2 + radius * Math.cos(angle) - 21),
      //    y = Math.round(height/2 + radius * Math.sin(angle) - $(this).find("a").outerHeight()/2);
	y = Math.round(height/2 + radius * Math.sin(angle) - 21);
