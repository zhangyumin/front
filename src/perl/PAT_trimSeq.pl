#!/usr/bin/perl -w
#按polyA_site_in_genome和strand截取位点序列
#输入表: test 或 test chromosome
#输出文件: intron_tagsnum2_400nts 或 _C_intron_400nt
##序列标题:字段名[字段值];
##1) 设置要取的uPA的条件:
##   1)by=grp_ftr,则读取4种grp_ftr, 允许设置utag,grp_ftr
##   2)by=class, 则读取4种class,允许设置class,grp_ftr
##   对于每条chr
##2) 若strand=+ 取上游300,1,下游99, 
##	 若strand=- 取上游99,1,下游300,再反转互补
##3) 若文件已存在则追加.

#(最后位点在第301)

#  修改历史
#  20091128: 文件名中添加_xxs表示序列数,如test_painfos_20_grp_three_prime_UTR_tagsnum2_400nt_154s
#  2009/12/28: 增加by all的选项,可以不分grp_ftr或class输出全部序列

#2010/6/4
#增加-from/-to选项,使可以截取任意范围. 如 1~100截取PA下游1~100nt,不含PA. -100~-1,截取上游. 默认-300~99,截取上游300,下游99,再包含PA

#2011/3/24
#大整改，用byfld,byval,cond取代原来的选项
#允许多个字段组合输出，如-byfld grp_ftr:class:strand
#修改输出序列的文件名及标题，只用>chr,strand,polyA_pos表示,如 >Chr1;-;12453928
...
#2011/5/10
#增加flds:chr:strand:coord选项

#2012-08-28
#增加suf选项，允许加一个后缀标识

#2012-09-11 Mtr修改 -- 需要longchr

#2013-12-22
#增加opath选项（默认为getTmpPath)
...
if ($byval) {
  @byvals=split(/:/,$byval);
  if ($#byvals<$#byflds) { #若 -byvals 3utr::，则自动补上两格空的
...
#确定输出文件信号:filesInfo[suffix,seqnum]
my ($suffix);
my $fldEnum; #二维矩阵，每行存放一个fld的枚举值
my $nf=1; #输出文件计数
if ($#byflds>=0) {
  for my $i(0..$#byflds) {
   if ($byvals[$i] eq '') { 
...
if ($#byflds<0) {
  ...
  my $condtxt;
  my $permute=getPermute($fldEnum); #得到如 ([3UTR,C],[3UTR,M]..)
  for my $i(0..$#$permute) {
	 $condtxt="and (";
     if ($cond ne '') {
...
#按每chr 遍历polyA_site_in_genome,trim400nt
#处理longchr
my (@chrs,$sqlchr);
if ($longchr) {
...
if ($longchr) { #Mtr
	...
	for my $i (0..$#$filesInfo) {
	  ...
	  for my $j(0..$#$idxsChr) {
        ...
        $num+=trimAll($tbl,$idxsChr->[$j][0],$idxsChr->[$j][1]); #遍历整个表来截取序列
	  }
	  print "$num\n";	
	}
} #long
...
#  trimAll(): 遍历整个表来截取序列
#  useage: trimAll();    
#  说明: 使用和修改全局参数 (chrfas,curFileIdx,num...)
#############################################################################
sub trimAll {
	my ($tbl,$is,$ie)=@_;
